...
void main() {
  ...
  test('normaliza texto removendo acentos e pontuação', () {
    expect(
      TextNormalizer.normalize('Café, pão & leite!'),
      'cafe pao leite',
    );
  });
  ...
  test('faz match por termo mais longo quando o nome contém um termo conhecido', () async {
  ...
  test('prioriza a categoria aprendida pelo usuário sobre o seed dictionary', () async {
    final repository = _FakeLearnedCategoryRepository();
    final learnedService = CategoryService(learnedCategoryRepository: repository);
  ...
  test('persiste categoria aprendida no repositório', () async {
    ...
    expect(repository.categories['detergente liquido'], ItemCategory.limpeza.name);
  });
}
